...
void CardPixmap::mouseDoubleClickEvent(QGraphicsSceneMouseEvent *event) {
     // qDebug() << "slot_insert_line(line, qMakePair(" << board_position.first << "," << board_position.second << "));";
      qDebug() << "Double Click" << this->entry_number();
...
void CardPixmap::mousePressEvent(QGraphicsSceneMouseEvent *event) {
...
void CardPixmap::mouseMoveEvent(QGraphicsSceneMouseEvent *event) {
      this->setPos(QPointF(_design_item_pos + this->_design_mouse_delta(event->scenePos())));
   }

void CardPixmap::mouseReleaseEvent(QGraphicsSceneMouseEvent *event) {
      if (event->button() == Qt::LeftButton) {
            //this->_design_mouse_press_pos = event->scenePos();
